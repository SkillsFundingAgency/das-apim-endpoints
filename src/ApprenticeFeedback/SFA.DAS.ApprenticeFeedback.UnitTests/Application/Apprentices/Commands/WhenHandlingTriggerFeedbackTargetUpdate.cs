...
namespace SFA.DAS.ApprenticeFeedback.UnitTests.Application.Apprentices.Commands
{
    public class WhenHandlingTriggerFeedbackTargetUpdate
    {
        ...
        [Test, MoqAutoData]
        public async Task Then_RequestForApprenticeFeedbackTargetsIsSent(TriggerFeedbackTargetUpdateCommand command)
        {
            // Act
            var result = await _handler.Handle(command, CancellationToken.None);

            // Assert
            //_mockFeedbackApiClient.Verify(c => c.GetAll<ApprenticeFeedbackTarget>(It.Is<GetAllApprenticeFeedbackTargetsRequest>(x => x.ApprenticeId == command.)), Times.Once);
        }
