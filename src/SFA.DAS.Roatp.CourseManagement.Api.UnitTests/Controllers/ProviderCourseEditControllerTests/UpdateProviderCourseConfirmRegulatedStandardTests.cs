...
{
    [TestFixture]
    public class UpdateProviderCourseConfirmRegulatedStandardTests
    {
        [Test, MoqAutoData]
        public async Task UpdateProviderCourseConfirmRegulatedStandard_Success_ReturnsNoContent(
            ...
            int ukprn, int larsCode, UpdateConfirmRegulatedStandardCommand command)
        {
            mediator.Setup(m => m.Send(It.Is<UpdateConfirmRegulatedStandardCommand>(c => c.Ukprn == ukprn && c.LarsCode == larsCode), It.IsAny<CancellationToken>())).ReturnsAsync(HttpStatusCode.NoContent);

            var result = await sut.UpdateProviderCourseConfirmRegulatedStandard(ukprn, larsCode, command);
        ...
        public async Task UpdateProviderCourseConfirmRegulatedStandard_Failed_ReturnsRespectiveStatusCode(
            ...
            int ukprn, int larsCode, UpdateConfirmRegulatedStandardCommand command)
        {
            mediator.Setup(m => m.Send(It.Is<UpdateConfirmRegulatedStandardCommand>(c => c.Ukprn == ukprn && c.LarsCode == larsCode), It.IsAny<CancellationToken>())).ReturnsAsync(HttpStatusCode.BadRequest);
