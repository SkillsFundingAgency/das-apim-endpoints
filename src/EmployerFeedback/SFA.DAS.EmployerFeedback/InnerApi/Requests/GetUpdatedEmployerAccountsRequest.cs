...
namespace SFA.DAS.EmployerFeedback.InnerApi.Requests
{
    public class GetUpdatedEmployerAccountsRequest : IGetApiRequest
    {
        ...
        public string GetUrl =>
            SinceDate.HasValue
                ? $"api/accounts/updated?sinceDate={SinceDate.Value.ToUniversalTime():yyyy-MM-ddTHH:mm:ss.fffffffZ}&pageNumber={PageNumber}&pageSize={PageSize}"
                : $"api/accounts/updated?pageNumber={PageNumber}&pageSize={PageSize}";
