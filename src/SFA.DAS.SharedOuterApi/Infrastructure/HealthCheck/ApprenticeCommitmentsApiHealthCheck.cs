...
namespace SFA.DAS.SharedOuterApi.Infrastructure.HealthCheck
{
    public class ApprenticeCommitmentsApiHealthCheck : IHealthCheck
    {
        ...
        public ApprenticeCommitmentsApiHealthCheck(IApprenticeCommitmentsApiClient<ApprenticeCommitmentsApiConfiguration> apiClient, ILogger<ApprenticeCommitmentsApiHealthCheck> logger)
        {
        ...
        {
            _logger.LogInformation("Pinging ApprenticeCommitments API");
            ...
            if ((int)response == 200)
            {
                ...
                _logger.LogInformation($"ApprenticeCommitments API ping successful and took {durationString}");

                return HealthCheckResult.Healthy(HealthCheckResultDescription,
                    new Dictionary<string, object> { { "Duration", durationString } });
            }

            _logger.LogWarning($"ApprenticeCommitments API ping failed : [Code: {response}]");
