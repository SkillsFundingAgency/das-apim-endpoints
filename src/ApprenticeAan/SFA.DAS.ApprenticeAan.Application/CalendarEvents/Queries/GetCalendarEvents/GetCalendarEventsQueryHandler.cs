...
public class GetCalendarEventsQueryHandler : IRequestHandler<GetCalendarEventsQuery, GetCalendarEventsQueryResult?>
{
    ...
    {
        return await _apiClient.GetCalendarEvents(request.RequestedByMemberId.ToString(), request.FromDate!, request.ToDate!, request.EventFormat!, request.CalendarIds!, request.RegionIds!, request.Page!, request.PageSize!, cancellationToken);
    }
}
