...
public static class GqlVacancyMapper
{
    ...
    private static SharedOuterApi.Types.Domain.Domain.AvailableWhere? MapEmployerLocationOption(IAllVacancyFields source, List<Address> employerLocations)
    {
        if (source is { EmployerLocationOption: not null })
        {
            return NullOrEnum<SharedOuterApi.Types.Domain.Domain.AvailableWhere>(source.EmployerLocationOption.ToString());
        }
