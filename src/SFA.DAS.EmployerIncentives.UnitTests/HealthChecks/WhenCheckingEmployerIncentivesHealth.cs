...
namespace SFA.DAS.EmployerIncentives.UnitTests.HealthChecks
{
    public class WhenCheckingEmployerIncentivesHealth
    {
        ...
        {
            employerIncentivesService.Setup(x => x.IsHealthy(CancellationToken.None)).ReturnsAsync(true);
        ...
        public async Task Then_Then_The_Service_Is_Called_And_UnHealthy_Returned_If_False(
            ...
            EmployerIncentivesHealthCheck healthCheck)
        {
            employerIncentivesService.Setup(x => x.IsHealthy(CancellationToken.None)).ReturnsAsync(false);
