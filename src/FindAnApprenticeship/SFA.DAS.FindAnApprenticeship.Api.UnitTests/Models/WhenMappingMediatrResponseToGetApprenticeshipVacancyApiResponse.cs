...
public class WhenMappingMediatrResponseToGetApprenticeshipVacancyApiResponse
{
    ...
    [Test, AutoData]
    public void Then_If_There_Is_No_Course_Default_Added(GetApprenticeshipVacancyQueryResult source)
    {
        source.CourseDetail = null;
            
        var result = (GetApprenticeshipVacancyApiResponse)source;
            
        result.CourseSkills.Should().BeEmpty();
        result.CourseOverviewOfRole.Should().BeEmpty();
        result.StandardPageUrl.Should().BeEmpty();
        result.CourseCoreDuties.Should().BeEmpty();
        result.CourseSkills.Should().BeEmpty();
    }
}