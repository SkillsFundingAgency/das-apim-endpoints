...
public class DeleteProviderCourseLocationRequestTests
{
    ...
    public void ImplicitOperator_ConstructsRequest(DeleteProviderCourseLocationCommand command)
    {
        ...
        request.DeleteUrl.Should().Be($"/providers/{command.Ukprn}/courses/{command.LarsCode}/locations/{command.Id}?userId={HttpUtility.UrlEncode(command.UserId)}&userDisplayName={HttpUtility.UrlEncode(command.UserDisplayName)}");
    }
}
