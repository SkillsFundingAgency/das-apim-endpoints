...
public class ProviderCoursesControllerGetAllProviderCoursesTests
{
    ...
    [Test]
    public async Task GetAllProviderCourses_HandlerReturnsEmptyList_ReturnsOkAndEmptyList()
    {
        var mediatorMock = new Mock<IMediator>();
        mediatorMock.Setup(m => m.Send(It.IsAny<GetAllProviderCoursesQuery>(), It.IsAny<CancellationToken>())).ReturnsAsync([]);
        var subject = new ProviderCoursesController(Mock.Of<ILogger<ProviderCoursesController>>(), mediatorMock.Object);

        var response = await subject.GetAllProviderCourses(ValidUkprn, CourseType.Apprenticeship);

        var okResult = response as OkObjectResult;
        var result = okResult.Value as List<GetAllProviderCoursesQueryResult>;
