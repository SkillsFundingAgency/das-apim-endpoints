...
namespace SFA.DAS.RoatpCourseManagement.Api.UnitTests.Controllers.ProviderCoursesControllerTests
{
    ...
    {
        const string ValidLarsCode = "101";
        private const int ValidUkprn = 10000001;
        [TestCase(ValidUkprn, "", 400)]
        [TestCase(0, "1", 400)]
        [TestCase(-1, "1", 400)]
        [TestCase(ValidUkprn, ValidLarsCode, 200)]
        public async Task GetProviderCourse_ReturnsExpectedState(int ukprn, string larsCode, int expectedStatusCode)
        {
