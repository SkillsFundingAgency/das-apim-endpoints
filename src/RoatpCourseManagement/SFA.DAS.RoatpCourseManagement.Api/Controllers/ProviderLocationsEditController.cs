...
public class ProviderLocationsEditController : ControllerBase
{
    ...
    {
        _logger.LogInformation("Outer API: Request to update provider location details for ukprn: {Ukprn} id: {LocationId}", ukprn, id);
        command.Ukprn = ukprn;
        command.Id = id;
        var httpStatusCode = await _mediator.Send(command);
        if (httpStatusCode != HttpStatusCode.NoContent)
        {
            _logger.LogError("Outer API: Failed request to update provider location details for ukprn: {Ukprn} id: {LocationId} with HttpStatusCode: {HttpStatusCode}", ukprn, id, httpStatusCode);
