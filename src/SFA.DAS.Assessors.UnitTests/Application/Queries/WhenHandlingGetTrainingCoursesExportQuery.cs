...
namespace SFA.DAS.Assessors.UnitTests.Application.Queries
{
    public class WhenHandlingGetTrainingCoursesExportQuery
    {
        ...
        {
            mockApiClient
                .Setup(client => client.Get<GetStandardsExportListResponse>(It.IsAny<GetAllStandardsRequest>()))
                .ReturnsAsync(apiResponse);

            var result = await handler.Handle(query, CancellationToken.None);

            result.TrainingCourses.Should().BeEquivalentTo(apiResponse.Courses);
        }
    }
}