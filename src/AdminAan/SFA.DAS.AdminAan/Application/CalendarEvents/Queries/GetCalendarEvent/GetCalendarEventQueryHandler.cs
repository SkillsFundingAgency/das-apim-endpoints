    ...
    : IRequestHandler<GetCalendarEventQuery, GetCalendarEventQueryResult>
{
    ...
    public async Task<GetCalendarEventQueryResult> Handle(GetCalendarEventQuery request, CancellationToken cancellationToken)
    {
        ...
        var result = new GetCalendarEventQueryResult
        {
            ...
            Attendees = apiResponse.Attendees.Select(a => new QueryAttendee(a.MemberId, a.UserType, a.MemberName, a.AddedDate, a.CancelledDate)).ToList(),
            CancelledAttendees = apiResponse.CancelledAttendees.Select(ca => new QueryCancelledAttendee(ca.MemberId, ca.UserType, ca.MemberName, ca.AddedDate, ca.CancelledDate)).ToList(),
